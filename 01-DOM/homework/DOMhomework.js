...
const span = document.querySelector("#createdBy"); /// querySelector llamamos al id createdBy 
span.innerHTML +="Marcos";          //innerHTML para acceder al texto y se lo cambiamos 
...
function ToDo(description) {            // le creamos dos propiedades 
...
ToDo.prototype.completeToDo = function (){
  this.complete = !this.complete;    /// seteamos (cambiamos) el valor de complete de false a true
}
...
function buildToDo(todo, index) {  // todo: ToDo{description: "hola", cpmplete: false} index  = numero
  // Tu código acá: 
  ...
  toDoText.setAttribute('id', index);              // <span id: "numero"> hola </span>
...
function buildToDos(toDos) {
  ...
  // [ToDo{description: "hola", cpmplete: false} , ToDo{description: "hola", cpmplete: false}, ToDo{description: "hola", cpmplete: false}, ToDo{description: "hola", cpmplete: false}]


  // Tu código acá:                            // de donde sale todo,index ? son del metodo map ... todo es todo lo que tiene ToDO
                                                // adentro en este caso y index es el indice (primero toma a uno y despues al que sigue), index primero va a ser el primer todo y despues el segundo y asi ....
  let newArray= toDos.map(function(todo, index){   /// ltodo y index son nombres que pusimos nosotros todo para entender que hace
    return buildToDo(todo,index);                         // ... referencia a los elementos "ToDo" del array y index para indicar 
  });                                                         /// el indice .. primero 0 , despues 1 , 2......
 
  return newArray;       // guardamos el map (que retorna automaticamente un array) en la variable newArray y lo retornamos 
...
function addToDo() {
  // Tu código acá:
  let input = document.querySelector('#toDoInput'); // para pasarle el valor del input "toDoInput" primero hay que traerlo
  let newToDo = new ToDo(input.value);            // input.value es el valor que tiene input ... la funcion constructora ToDo
                                                    ///... recibia un string como argumento y ese estring va a ser input.value
  if(input.value !== ""){                                                 // y se lo va a asiganr a this.description 
  toDoItems.push(newToDo); // usamos push porque toDoItems es un array 
  input.value = "";       // valor del input toDoInput seteado en un string vacio y solo escribimos input porque es la variable que tiene guardado al input type
  displayToDos();
  }
}
...
function completeToDo(event) {
  ...
  const index = event.target.id;  /// event.target entiende donde hago click sobre quien estoy trabajando 
  toDoItems[index].completeToDo(); // toDoIems es un array .. index son los objeto que tiene el array dentro ... .complete es una propiedad de esos objetos 

  displayToDos(); /// actualizamos esto para actualizar lo que se ve en pantalla , las tarjetas 
}
